...
public class QuizManager : MonoBehaviour
{
    ...
    public void StartQuiz()
    {
        ...
        score = 0;
        questionIndex = 0;
        timer = 20f; // Reset timer when starting new quiz
        ShuffleAndSelectQuestions(); // Ensure a new set of questions
        UpdateScoreText();
        DisplayNextQuestion();
    }
    ...
    void DisplayNextQuestion()
    {
        ...
        if (questionIndex < selectedQuestions.Count)
        {
            ...
            timer = 20f; // Reset timer for the next question
            questionIndex++;
        }
        else
        {
    ...
    void EndGame()
    {
        ...
        if (score >= 250)
        {
            // Load a separate scene for high scores
            SceneManager.LoadScene("Intermediate_Scene");
        }
        else
        {
            endGamePanel.SetActive(true);
            endGameMessage.text = "Sorry, your score is not sufficient to claim the fuel and quantum circuits. Try again!";
        }
    }
